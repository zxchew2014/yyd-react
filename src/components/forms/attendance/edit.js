...
class EditAttendance extends React.Component {
    ...
    retrieveStatesList = async () => {
        const list = [];
        const StatesRef = firebase.database().ref('States').orderByKey();

        const snapshot = await StatesRef.once('value');
        const val = snapshot.val();
        if (!val) return;

        const states = [].concat(...Object.values(val));
    ...
    retrieveSubjectList = async (level) => {
        const list = [];
        ...
        const subjects = [].concat(...Object.values(val));
    ...
    renderEditForm = () => {
        ...
        const STUDENT_LIST = Array.isArray(data.students)
            ...
            const STUDENT_LIST_FIELD = (
                <Form.Field key={student.Id}>
                    <label htmlFor={studentName}>
                        {data.level === 'Primary' ?
                            `${number}. ${newStudentName} - P${student.Primary}` :
                            `${number}. ${newStudentName} - Sec${student.Secondary} - ${student.Group}`
                        }
                        {student.Batch && (
        ...
        const primaryStr =
            data.level === "Primary"
                ? data.primary.map(v => `P${v}`).join(', ')
                : '';


        const groupStr =
            data.level === "Secondary" && Array.isArray(data.group)
                ? data.group.join(', ')
                : '';
